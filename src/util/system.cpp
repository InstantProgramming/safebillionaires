...
const char * const BITCOIN_CONF_FILENAME = "safebillionaire.conf";
...
#else
    const char* pszModule = "safebillionaire";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Safebillionaire";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Safebillionaire";
#else
    // Unix
    return pathRet / ".safebillionaire";
#endif
#endif
}
