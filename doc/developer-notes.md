...
- [Development guidelines](#development-guidelines)
    - [General Safebillionaire Core](#general-safebillionaire-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Safebillionaire Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Safebillionaire Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/safebillionaire-project/safebillionaire/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/safebillionaired -printtoconsole
```
...
To profile a running safebillionaired process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep safebillionaired` -- sleep 60
```
...
Safebillionaire Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Safebillionaire Core, where everyone uses
...
pay attention to for reviewers of Safebillionaire Core code.

General Safebillionaire Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Safebillionaire Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Safebillionaire Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Safebillionaire Core so that they can be integrated
...
file descriptors it will cause problems with Safebillionaire's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Safebillionaire's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `safebillionaire-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `safebillionaire-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Safebillionaire Core, the wallet was always
