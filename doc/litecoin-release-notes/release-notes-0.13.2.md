Safebillionaire Core version 0.13.2 is now available from:

  <https://download.safebillionaire.org/safebillionaire-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a safebillionaire
...
ECDSA signatures inside Safebillionaire transactions now use validation using
...
Previous versions of Safebillionaire Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Safebillionaire Core was to raise the default minimum
relay fee.

Safebillionaire Core 0.13.2 will have a strict maximum size on the mempool. The
...
Safebillionaire Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running safebillionaired without having to do any manual configuration.
...
Safebillionaire Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Safebillionaire Core will relay transactions with
...
entirely in Safebillionaire Core 0.13, and it is at this time undecided whether the
...
Safebillionaire Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Safebillionaire Core automatically creates a hidden service to listen on, without
manual configuration. Safebillionaire Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Safebillionaire Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Safebillionaire
Core to use floating fees. By default, Safebillionaire Core will use floating
fees.
...
(default: `0.0002` SBLN/kB).

At all times, Safebillionaire Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SBLN.
Furthermore, Safebillionaire Core will never create transactions paying less than
...
`safebillionaire.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoinrpc>
        ...
        # optional bypass safebillionaired rpc basic auth
...
Restarting safebillionaired will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in safebillionaire.conf
...
safebillionaire-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/safebillionaire-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Safebillionaire Core code base has
...
- `safebillionaire-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `safebillionaire-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Safebillionaire scripting system that in combination with
...
Safebillionaire transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Safebillionaire Core.
...
The two phases are distinct in the Safebillionaire-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Safebillionaire wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Safebillionaire.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Safebillionaire block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Safebillionaire addresses (both P2PKH addresses
  ...
  believed to be provided by Safebillionaire's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Safebillionaire's current ledger (called the UTXO set),
  ...
  note that Safebillionaire Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Safebillionaire users to individually opt-in to almost any change in the Safebillionaire
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Safebillionaire Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Safebillionaire
client to Safebillionaire Core 0.13.2 to signal for segwit support. When a
...
Since Safebillionaire Core 0.10.0, nodes have defaulted to only relaying and
...
- `safebillionaire-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `safebillionaire-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
